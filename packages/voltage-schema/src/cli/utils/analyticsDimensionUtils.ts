...
export function getAllDimensions(options: GetAllDimensionsOptions = {}): DimensionData[] | Dimension[] {
  ...
  const allDimensions = new Map<string, AnalyticsGlobals["dimensions"][0]>();
  const allGroups: AnalyticsGlobals["groups"] = [];
  const allEvents: AnalyticsEvents["events"] = {};
  const allMeta: AnalyticsGlobals["meta"] = [];
